...
public class MedianBolt extends BaseWindowedBolt {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(MedianBolt.class);
  ...
  public void execute(TupleWindow window) {
    ...
    if (size > 0) {
      ...
      if (size % 2 == 0) {
        ...
        median = valueExtractor.apply(tuples.get(size / 2)).intValue();
      }
      collector.emit(shapeOutput.apply(window, median));
    }
  }
  ...
  public MedianBolt withValueExtractor(Function<Object, Number> valueExtractor) {
    this.valueExtractor = valueExtractor;
    return this;
  }
  
  public MedianBolt withOutputProducer(BiFunction<TupleWindow, Number, ArrayList<Object>> outputProducer) {
    this.shapeOutput = outputProducer;
    return this;
  }
  
  public MedianBolt withOutputFields(Fields outputFields) {
